...
class Modman {
		...
		try {
			...
			switch ($aParameters[1]) {
				...
				case 'list':
					...
					/* $Modman_Command = new Modman_Command_All('');
					foreach ($Modman_Command->getAllModules() as $sModuleName) {
						echo $sModuleName;
					}
					exit; */
				...
				case 'update':
					...
					$oUpdate = new Modman_Command_Update($aParameters[2]);
					$oUpdate->doUpdate($bForce);
					break;
				default:
	...
	public function printHelp(){
		$sHelp = <<< EOH
PHP-based module manager, originally implemented as bash-script (Mod by KhoaZero123)
...
- link [--force] 					link to a module that already exists on the local filesystem
...
- deploy [--force]					deploy an existing module (VCS not required or used)
- deploy-all [--force]				deploy all modules (no VCS interaction)
- clean								clean up broken symlinks (run this after deleting a module)
- create [--force, --include <include_file> and --include-hidden]
- clone [--force, --create-modman] checkout a new modman-compatible module using git clone
...
class Modman_Command_Update {
	 ...
	 * constructor
	 *
	 * @param string $sModuleName which module to update
	 * @throws Exception
	 */
	public function __construct($sModuleName) {
		if (empty($sModuleName)) {
			throw new Exception('please provide a module name to update');
		}
		$this->sModuleName = $sModuleName;
	}
	 ...
	 * @throws Exception on error
	 */
	public function doUpdate($bForce = false) {
		if ($this->sModuleName === Modman_Command_Init::MODMAN_BASEDIR_FILE) {
			return;
		}
		echo 'Updateting module '.$this->sModuleName.' ... ' . PHP_EOL;
		$oModmanModuleSymlink = new Modman_Module_Symlink($this->sModuleName);
		$sTarget = $oModmanModuleSymlink->getModmanModuleSymlinkPath();
		chdir($sTarget); // echo getcwd() . PHP_EOL;
		if($bForce) {
			$cmd = 'git fetch --force && git reset --hard HEAD';
		} else $cmd = 'git pull';
